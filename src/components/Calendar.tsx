...
type StaticEvent = { day: number; start: number; len: number; title: string; type: 'dead' | 'alive' };
...
// Spread saved meetings across calendar slots deterministically
...
export function Calendar({ onNew, rows = [], onEditRow }: Props) {
  // Build a map of (day, hour) -> saved row for quick lookup
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[56px_repeat(5,1fr)] text-xs">
        ...
        {HOURS.map((h) => (
          <Row key={h} hour={h} onNew={onNew} slotMap={slotMap} onEditRow={onEditRow} />
        ))}
      </div>
    </div>
  );
    ...
    <>
      ...
      {[0, 1, 2, 3, 4].map((day) => {
        ...
        return (
          ...
          >
            {/* Saved meeting event — clickable to re-interrogate */}
            {savedRow ? (
              <SavedEventBlock row={savedRow} onEdit={() => onEditRow?.(savedRow)} />
            ) : staticEv ? (
              <StaticEventBlock ev={staticEv} onNew={() => onNew({ day, hour })} />
            ) : (
...
function StaticEventBlock({ ev, onNew }: { ev: StaticEvent; onNew: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="px-2 pt-1 font-sans font-medium leading-tight"
      ...
      <div
        ...
        style={{ background: 'rgba(242,201,76,0.08)' }}
      >
        <span className="font-sans font-medium text-[10px] tracking-widest" style={{ color: '#f2c94c' }}>+ NEW</span>
      </div>
    </button>
  );
}
...
function SavedEventBlock({ row, onEdit }: { row: MeetingRow; onEdit: () => void }) {
  const isKilled = row.verdict === 'kill' || row.verdict === 'async';
  const wasAccepted = row.accepted;
  const dead = isKilled && wasAccepted;
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-0 right-0 px-1.5 font-sans font-medium"
        style={{
          fontSize: 7,
          letterSpacing: '0.12em',
          background: dead ? '#eb5757' : '#f2c94c',
          color: '#000',
        }}
      >
      ...
      {/* Hover overlay — interrogate again */}
      <div
